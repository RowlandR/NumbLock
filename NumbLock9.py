...
def RunLvl(LvlInd, LvlMode):
    ...
    Count1 = 0
    global RunLvlRec #records missed data during the excersize, problem, time, interaction, etc
    RunLvlRec = []
    Terminate1 = '' #to tell summary reason for termination
    ...
    while True:
        if Timer1 != 'x':
            ...
            if 60 - (Timer2 - Timer1) <= 0:
                print("Time's Up!")
                Terminate1 = 'Time Violation'
                break
        if Count1 == 25:
            print('All done!')
            Terminate1 = 'Completed Excersize'
            break
        ...
        if Answer1 != 'x': #x means that the operation called upon did not work
            ...
            try: #attempts to see if Answer2 is actually an int, if not, then it will look for other command prompts or false input
                ...
                Count1 += 1
                if Answer2 != Answer1:
                    RunLvlRec.append('Incorrect: ' + str(Num2) + Oper1 + str(Num1) + '=' + str(Answer1) + ', You Guessed: ' + str(Answer2))
                    #records incorrect question
            except:
                if Answer2.lower() == "end":
                    Terminate1 = 'User Ended Program'
                    break
                else:
                    print("Question Skipped")
                    RunLvlRec.append('Skip: ' + str(Num2) + Oper1 + str(Num1) + '=' + str(Answer1))
                    #records skipped question
                    Count1 += 1
    LvlSum(Hold1, LvlMode, ActUser, RunLvlRec, Terminate1, Count1, TimeHold) #Summary for the lvl
def LvlSum(LvlInd, LvlMode, LvlUser, LvlRec, Ter, Num, Time):
    #Lvl Mode (1, 2, f, m), Lvl Orientation (timed, relaxed), Active User, Results from lvl, Reason for termination #Questions Encountered #time left
    Summary = UserDash
    Hold1 = Num - len(LvlRec)
    if Num == 25:
        if Hold1 >= 23:
            PassFail = 'Lvl Passed'
        else:
        ...
        PassFail = 'Lvl Incompleted'
    if LvlInd == 'f':
        Summary += 'Lvl Style: Freestyle'
    elif LvlInd == 'm':
        Summary += 'Lvl Style: Missed'
    else:
        Summary += 'Lvl Style: Lvl ' + str(LvlInd)
        if PassFail == 'Lvl Passed' and LvlInd == ActLvl:
            PassFail += ', Lvl ' + str(ActLvl + 1) + ' Has Been Unlocked!'
    
    Summary += '\nLvl Mode: ' + str(LvlMode)
    if LvlMode == 'Timed':
        Summary += '\nTime Left: ' + str(Time) + ' Seconds'
    Summary += '\nReason For Termination: ' + Ter + '\nResult: ' + PassFail + '\n\nQuestions Missed (' + str(len(LvlRec)) + '):'
    if len(LvlRec) == 0:
        Summary += '\nNo Problems Incorrect or Skipped'
        if Num == 25:
            Summary += ', Good Work!'
    else:
        for i in LvlRec:
            Summary += '\n' + i
    input(Summary + '\n\nType Anthing To Continue...')
...
Brax1 = 0
while True: #The Major Access Loop
    while True:
        if Brax1 == 1:
            break
        Brax1 = 0
                ...
                if Hold1 == '%back':
                    ...
                    try: #trying to see if the user actually put in the correct input
                        if Hold1.lower() != 'freestyle':
                            Hold1 = str(int(Hold1[4:])) #the : after the '4' is used to see if the lvl has more than 1 digit
                            if int(Hold1) > ActLvl:
                                raise
                        else:
                            Hold1 = 'Freestyle'
                        ...
                        while True: #Lvl Mode Loop
                            ...
                            if Hold2 == '%back':
                                ...
                                if Hold3 == '%back':
                                    ...
                                    RunLvl(Hold1, Hold2) #Lvl Mode, Lvl Orientation, No Longer --> Active User (now user goes into summary)
                                    #summary is inside def of RunLvl()
                                    Brax1 = 1
                                    break
                            else:
