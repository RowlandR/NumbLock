...
def additem(Input, TarChar, Pos): #What you want inserted into file ||| The char(s) that you are searching for ||| Where in respect to the char you want the insert (takes and int, 0 is before)
    if TarChar == '%append':
        ...
        Hold4 = ''
        for i in Hold3:
            if len(TarChar) > 1: #checks to see if there is more than one char in TarChar
                if len(Hold4) >= len(TarChar): #checks if enough chars have actually been checked
                    Hold4 = Hold4[1:] #deletes the first char
                Hold4 += i # adds a char
            else:
                Hold4 = i #if only one TarChar char exists
            if Hold4 == TarChar: #found the TarChar in code
                with open('Mem-NL.txt', 'w') as W: #writing to file
                    #W.write(Hold3[:Count1 - len(TarChar) + 1] + Input + Hold3[Count1 - len(TarChar) + 1:]) #doesn't include the Pos var
                    W.write(Hold3[:Count1 + Pos - len(TarChar) + 1] + Input + Hold3[Count1 + Pos - len(TarChar) + 1:])
                    Count1 = 'x'
                    break
            Count1 += 1 #adds more to Count1 so that it can be traced back to the first instance of the TarChar
        if Count1 != 'x':
        ...
        """#This would work only if the TarChar was always 1 character
        for i in Hold1:
            if i == TarChar:
                Hold2 = Hold1[:Count1] + Input + Hold1[Count1:]
...
do the 23/25 questions in 1 minute, 'Relaxed', which means there is
no time limit, or 'Study', which means that you will ben given all 'Missed'
problems and problems pertaining to that lvl.
...
"""
#### Used to determine if '$Users$@^' exists
finditem('$', '', 'Users', '@', '^')
if Return1 == '':
...
while True: #The Access Loop
                        ...
                        if Brax1 == 3:
                            ...
                            while True: #Used to verify if the Verified Password meets the specifications
                                ...
                                if Hold3 == '%back':
                                    ...
                                    additem('\n$User-' + Hold1 +'$ #' + Hold2 + '# *1*^', '%append', '%NULL')
                                    additem(' #' + Hold1 + '#', '@', 0)
                                    break
                                else:
    ...
    ActUser = Hold1
    ActPass = Hold2
    #### This is just used to create a header (dashes) with the name in it
    
    #--------------------------
    #abcdefghijklmnopqrstuvwxyz
    #ABCDEFGHIJKLMNOPQRSTUVWXYZ --> (just to check that each character is the same length) All characters amount to the same length in Python
    #,,,,,,,,,,,,,,,,,,,,,,,,,,
    #iiiiiiiiiiiiiiiiiiiiiiiiii
    
    #Hold1 = 78 (number of dashes) - len(ActUser) (Length of Users Name) - 8 (chars in " User: " + " ")
    ...
    while True:  #The User Loop
        ...
        if Hold1 == 'Start':
            ...
            while True:
                ...
                Hold1 = input()
                if Hold1 == '%back' or Hold1 == 'Freestyle' or Hold1 == 'Missed' or Hold1[0:3] == 'Lvl' or Hold1[0:3] == 'LVL' or Hold1[0:3] == 'lvl':
                    break
                else:
            ...
            if Hold1 == 'Freestyle':
                ...
                pass
            elif Hold1[0:3] == 'Lvl' or Hold1[0:3] == 'LVL' or Hold1[0:3] == 'lvl':
                pass
        elif Hold1 == 'Help':
            ...
            if Hold1 == 'Yes' or Hold1 == 'YES' or Hold1 == 'yes': #Just to make sure that the many variations of 'Yes' will actually work
                ActUser, ActPass, ActLvl, Hold1, Hold2,  = '', '', '', '', ''
                break
