...
def RunLvl(LvlInd, LvlMode, ActUser): #What lvl (Freestyle, Missed, or Lvl-%) ||| Relaxed or Timed Mode ||| What user is active (for recording score purposes)
    ...
    for i in range(3):
        print(3 - i)
        time.sleep(1)
    #Create Timer #Create Actual Game #Make countdown more efficient
    while True:#creates a problem #Num1 = first number, #Num2 = second number, #Oper1 = Operation, #Answer1 = answer of equation #Answer2 = their input
        ...
        #format: print(str(Num2)  + ' ' + Oper1 + ' ' + str(Num1) + ' = ')
        # other kind of format: print(str(Num2) + Oper1 + str(Num1) + '=') #use this format
        """ #this dhould only be used if the answer is incredibly long, like some crazy decimal
        elif Oper2 == '3':
            Oper1 = '/'
            Answer1 = Num2 / Num1
        """
        ...
        elif Oper1 == 5: #abnormal
            try: #python does not like when you divide by zero
                Answer1 = Num2 // Num1
                Oper1 = '//'
            except:
                Oper1 = 'x'
        elif Oper1 == 6: #normal I think, if Num2 > Num1, than the answer is just Num2, that is how it normally works
            Oper1 = '%'
            #Oper1 = 'mod'
            Answer1 = Num2 % Num1
        else:
            print("darn it")
        if Oper1 != 'x': #x means that the operation called upon did not work
            Answer2 = input(str(Num2) + Oper1 + str(Num1) + '=')
            if int(Answer2) == int(Answer1):
                print('Correct!')
            else:
                print('Incorrect')
